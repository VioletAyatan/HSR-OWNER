...
> **完整验收：用户确认第六轮导出成功，日志核实 finished（691503ms、64139 次文件发布），无 Error/序列化失败/内存保护；采样私有提交 4983～5386 MiB，调用历史稳定 262 KiB。** 空表、不存在路径及未支持类型等覆盖限制已核对并记录，不能把流程完成解读为全部原始资源无遗漏。当前结果见[交接文档第 0.6 节](Resources排查工作流与交接.md)。以下各轮更新保留为历史。

> **第六轮更新：用户确认内存不再无限增长，约 1.92 亿次调用后历史缓存仍为 262 KiB、private_mib=5506。此次在 37771 次文件发布后因字幕空路径失败；现场 167 条空 CaptionPath 已纳入跳过处理和日志，公共入口也过滤空引用。16 项 Dumper 测试与构建通过，待复测。** 最新状态见[交接文档第 0.5 节](Resources排查工作流与交接.md)。

> **第五轮更新：发现 get_native_method 将每次反射方法名永久追加到全局诊断 Vec，已改成有界复用循环缓存；百万次调用测试保持约 262 KiB，相关 16 项测试及 DLL 构建通过。** 这是明确的 Rust 增长点，不能继续只围绕 GC 推测；游戏内效果仍待验证。最新状态见[交接文档第 0.4 节](Resources排查工作流与交接.md)。

> **第四轮更新：第三轮复测在 Config 触发 16 GiB 提交保护（采样值 21280 MiB），用户观察物理内存 100%。已增加物理内存检查、阶段内存差值及多路径表单次序列化复用；13 项 Dumper 测试和构建通过。JSON 对象写完即释放，但托管 GC 与游戏缓存仍待调查。** 最新状态见[交接文档第 0.3 节](Resources排查工作流与交接.md)。

> **第三轮更新：第二轮游戏内复测在 SpecialAvatarRelicMainValue 的 MoveNext 收到托管异常，1985 次文件发布后正常 Failed，private_mib=9076，未触发内存保护。已补充异常类型/存储消息及枚举器值类型适配，Dumper 12 项测试和构建通过，仍待复测确认根因。** 以[交接文档第 0.2 节](Resources排查工作流与交接.md)为当前状态。
